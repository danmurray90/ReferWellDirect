...
class ProbabilityCalibrationService:
    ...
    def get_calibration_metrics(self, scores: np.ndarray, labels: np.ndarray) -> Dict[str, float]:
        """
...
class VectorEmbeddingService:
    ...
    def update_psychologist_embedding(self, psychologist: Psychologist) -> bool:
        ...
        try:
            ...
            if not text.strip():
                self.logger.warning(f"No text content for psychologist {psychologist.id}")
                return False
            ...
            self.logger.error(f"Failed to update embedding for psychologist {psychologist.id}: {e}")
            return False
    ...
    def search(
        ...
        try:
            ...
            if not self.bm25_service.build_index(psychologists):
                self.logger.warning("Failed to build BM25 index, falling back to vector only")
                return self._vector_only_search(query, psychologists, top_k)
            
            # Get BM25 results
            bm25_results = self.bm25_service.search(query, top_k * 2)  # Get more for better coverage
            ...
            for psychologist in psychologists:
                if psychologist.embedding:
                    try:
                        ...
                        self.logger.warning(f"Failed to get embedding for psychologist {psychologist.id}: {e}")
                        continue
            ...
            for psych_id, score in top_results:
                try:
                    ...
                    self.logger.warning(f"Psychologist {psych_id} not found in queryset")
                    continue
...
class MatchingService:
    ...
    def __init__(self, vector_weight: float = 0.7, bm25_weight: float = 0.3, use_calibration: bool = True):
        """
        ...
        self.vector_service = VectorEmbeddingService()
        self.calibration_service = ProbabilityCalibrationService() if use_calibration else None
        self.logger = logging.getLogger(f"{__name__}.{self.__class__.__name__}")
    ...
    def _calculate_specialism_score(
        ...
        preferred_score = preferred_matches / total_preferred if total_preferred > 0 else 1.0
    ...
    def _apply_structured_filters(
        ...
        for match in matches:
            ...
            if referral.required_specialisms:
                if not any(spec in psychologist.specialisms for spec in referral.required_specialisms):
                    continue
            
            # Apply language filter
            if referral.language_requirements:
                if not any(lang in psychologist.languages for lang in referral.language_requirements):
                    continue
    ...
    def train_calibration_model(
        ...
        try:
            ...
            if success:
                self.logger.info(f"Trained {calibration_type} calibration model on {len(training_data)} samples")
                
                # Calculate and log metrics
                metrics = self.calibration_service.get_calibration_metrics(scores, labels)
    ...
    def get_calibration_metrics(self, test_data: List[Dict[str, Any]]) -> Dict[str, float]:
        """
