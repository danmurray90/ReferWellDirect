...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            '--referral-id',
            type=str,
            help='Specific referral ID to test (optional)'
        )
        parser.add_argument(
            '--create-thresholds',
            action='store_true',
    ...
    def handle(self, *args, **options):
        self.stdout.write(self.style.SUCCESS('Testing threshold routing functionality...'))
        ...
        if options['referral_id']:
            ...
            if referral:
                ...
                self.stdout.write(
                    self.style.WARNING('No test referrals found. Create a referral first.')
                )
    ...
    def create_default_thresholds(self):
        ...
        if success:
            ...
            self.stdout.write(
                self.style.ERROR('Failed to create default thresholds')
            )
    ...
    def show_routing_statistics(self):
        ...
        self.stdout.write(f"Auto-routed: {stats['auto_routed']} ({stats['auto_percentage']:.1f}%)")
        self.stdout.write(f"High-touch routed: {stats['high_touch_routed']} ({stats['high_touch_percentage']:.1f}%)")
        self.stdout.write(f"Manual review: {stats['manual_review']} ({stats['manual_percentage']:.1f}%)")
    ...
    def test_referral_routing(self, referral):
        ...
        self.stdout.write(f'High-touch threshold: {routing_decision["high_touch_threshold"]:.3f}')
        ...
        if referral.is_high_touch_queue:
            self.stdout.write(
                self.style.WARNING('Referral routed to High-Touch queue for manual review')
            )
        elif referral.status == Referral.Status.SHORTLISTED:
            self.stdout.write(
                self.style.SUCCESS('Referral auto-routed to shortlisted')
            )
        else:
            self.stdout.write(
                self.style.WARNING('Referral requires manual review')
            )
