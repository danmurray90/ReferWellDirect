...
from matching.services import BM25Service, MatchingService, VectorEmbeddingService
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--test-caching", action="store_true", help="Test caching functionality"
        )
        parser.add_argument(
            "--test-embeddings",
            action="store_true",
        ...
        parser.add_argument(
            "--test-bm25", action="store_true", help="Test BM25 search performance"
        )
        parser.add_argument(
            "--test-matching",
            action="store_true",
        ...
        parser.add_argument(
            "--clear-cache", action="store_true", help="Clear all caches before testing"
        )
    ...
    def handle(self, *args, **options):
        ...
        if not any(
            [
                options["test_caching"],
                options["test_embeddings"],
                options["test_bm25"],
                options["test_matching"],
            ]
        ):
    ...
    def test_caching(self):
        ...
        if psychologists.exists():
            ...
            if second_call_time > 0:
                self.stdout.write(
                    f"Speed improvement: {first_call_time/second_call_time:.2f}x"
                )
    ...
    def test_bm25(self):
        ...
        for query in test_queries:
            ...
            self.stdout.write(
                f"Query '{query}': {query_time:.4f}s ({len(results)} results)"
            )
    ...
    def test_matching(self):
        ...
        if cached_time > 0:
            self.stdout.write(
                f"Cache speed improvement: {matching_time/cached_time:.2f}x"
            )
    ...
    def get_cache_stats(self):
        ...
        try:
            ...
            embedding_keys = (
                len([k for k in keys if k.startswith("embedding_")]) if keys else 0
            )
            bm25_keys = len([k for k in keys if k.startswith("bm25_")]) if keys else 0
            threshold_keys = (
                len([k for k in keys if k.startswith("threshold_config_")])
                if keys
                else 0
            )
