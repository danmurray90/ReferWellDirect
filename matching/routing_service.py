...
class ReferralRoutingService:
        ...
        try:
            with transaction.atomic():
                ...
                if decision == 'auto':
                    ...
                    self.logger.info(f"Auto-routed referral {referral.id} to shortlisted")
                    
                elif decision == 'high_touch':
                    # High-touch route: needs human review
                    referral.status = Referral.Status.HIGH_TOUCH_QUEUE
                    self.logger.info(f"High-touch routed referral {referral.id} to queue")
                    
                else:  # manual_review
                    # Manual review: needs human intervention
                    referral.status = Referral.Status.MATCHING
                    self.logger.info(f"Manual review required for referral {referral.id}")
                ...
                referral.routing_metadata.update({
                    ...
                    'routed_at': referral.updated_at.isoformat() if referral.updated_at else None
                })
                
                referral.save(update_fields=['status', 'routing_metadata', 'updated_at'])
                
                self.logger.info(f"Successfully routed referral {referral.id} with decision: {decision}")
                return True
    ...
    def get_routing_statistics(self) -> Dict[str, Any]:
        ...
        stats = Referral.objects.aggregate(
            ...
            high_touch_routed=Count('id', filter=Q(status=Referral.Status.HIGH_TOUCH_QUEUE)),
        ...
        return {
            ...
            'auto_percentage': (stats['auto_routed'] / stats['total_referrals'] * 100) if stats['total_referrals'] > 0 else 0,
            'high_touch_percentage': (stats['high_touch_routed'] / stats['total_referrals'] * 100) if stats['total_referrals'] > 0 else 0,
            'manual_percentage': (stats['manual_review'] / stats['total_referrals'] * 100) if stats['total_referrals'] > 0 else 0,
        }
        ...
        try:
            ...
            for threshold_data in default_thresholds:
                MatchingThreshold.objects.get_or_create(
                    user_type=threshold_data['user_type'],
