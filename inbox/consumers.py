...
class NotificationConsumer(AsyncWebsocketConsumer):
    ...
    def create_notification_channel(self):
        ...
        try:
            ...
            logger.info(f"Notification channel {'created' if created else 'updated'}: {self.channel_name}")
...
class NotificationGroupConsumer(AsyncWebsocketConsumer):
    ...
    async def connect(self):
        ...
        # Join group
        await self.channel_layer.group_add(
            f"group_{self.group_name}",
            self.channel_name
        )
        
        await self.accept()
        
        logger.info(f"WebSocket connected to group {self.group_name} for user {self.user.id}")
    ...
    async def disconnect(self, close_code):
        ...
        if hasattr(self, 'user') and self.user.is_authenticated:
            ...
            logger.info(f"WebSocket disconnected from group {self.group_name} for user {self.user.id}")
