...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            f"Creating test notifications for user: {user.get_full_name()}"
        )
        ...
        for i in range(count):
            ...
            notification = service.create_notification(
                ...
                referral=referral
                if current_type in ["referral_update", "matching_complete"]
                else None,
            )
        ...
        if created_notifications:
            ...
            if service.mark_as_read(str(first_notification.id), user):
                self.stdout.write(
                    self.style.SUCCESS(
                        f"Marked notification as read: {first_notification.title}"
                    )
                )
            else:
                self.stdout.write(
        ...
        if notification_type == Notification.NotificationType.REFERRAL_UPDATE:
            ...
            return {
                **base_data,
                ...
                "is_important": index % 3
                == 0,  # Make every 3rd system notification important
            }
