...
class ReferralForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            'presenting_problem': forms.Textarea(attrs={'rows': 4, 'class': 'form-control'}),
            ...
            'preferred_latitude': forms.NumberInput(attrs={'class': 'form-control', 'step': 'any'}),
            'preferred_longitude': forms.NumberInput(attrs={'class': 'form-control', 'step': 'any'}),
            'max_distance_km': forms.NumberInput(attrs={'class': 'form-control', 'min': 1, 'max': 500}),
    ...
    def __init__(self, *args, **kwargs):
        ...
        if user and user.is_gp:
            ...
            self.fields['patient'].queryset = User.objects.filter(user_type=User.UserType.PATIENT)
