...
class ReferralSerializer(serializers.ModelSerializer):
    ...
    referrer_name = serializers.CharField(
        source="referrer.get_full_name", read_only=True
    )
    ...
    def validate_max_distance_km(self, value):
        ...
        if value < 1 or value > 500:
            raise serializers.ValidationError(
                "Maximum distance must be between 1 and 500 km."
            )
        return value
...
class CandidateSerializer(serializers.ModelSerializer):
    ...
    psychologist_name = serializers.CharField(
        source="psychologist.get_full_name", read_only=True
    )
    psychologist_email = serializers.CharField(
        source="psychologist.email", read_only=True
    )
    status_display = serializers.CharField(source="get_status_display", read_only=True)
