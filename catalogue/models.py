...
class Psychologist(models.Model):
    ...
    title = models.CharField(
        max_length=50, blank=True, help_text="Professional title (Dr., Prof., etc.)"
    )
    qualifications = models.JSONField(
        default=list, blank=True, help_text="List of qualifications"
    )
    specialisms = models.JSONField(
        default=list, blank=True, help_text="List of specialisms"
    )
    languages = models.JSONField(default=list, help_text="Languages spoken")

    # Service information
    service_type = models.CharField(
        max_length=10, choices=ServiceType.choices, default=ServiceType.MIXED
    )
    ...
    registration_number = models.CharField(
        max_length=50, blank=True, help_text="Professional registration number"
    )
    registration_body = models.CharField(
        max_length=100, blank=True, help_text="Registration body (HCPC, BPS, etc.)"
    )
    ...
    current_patients = models.PositiveIntegerField(
    ...
    preferred_age_groups = models.JSONField(
        default=list, blank=True, help_text="Preferred patient age groups"
    )
    preferred_conditions = models.JSONField(
        default=list, blank=True, help_text="Preferred conditions to treat"
    )
    ...
    embedding = models.TextField(
        null=True, blank=True, help_text="Vector embedding for similarity matching"
    )
    ...
    def is_available(self):
        ...
        return (
            ...
            and self.is_active
            and self.is_accepting_referrals
            and self.capacity_available
        )
...
class Availability(models.Model):
    ...
    psychologist = models.ForeignKey(
        Psychologist, on_delete=models.CASCADE, related_name="availabilities"
    )
    ...
    modality = models.CharField(
        max_length=10,
        choices=Psychologist.Modality.choices,
        default=Psychologist.Modality.IN_PERSON,
    )
...
class Qualification(models.Model):
    ...
    abbreviation = models.CharField(
        max_length=20, blank=True, help_text="Common abbreviation (e.g., 'DClinPsy')"
    )
...
class Review(models.Model):
    ...
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    psychologist = models.ForeignKey(
        Psychologist, on_delete=models.CASCADE, related_name="reviews"
    )
