...
from .models import Psychologist, Availability, Specialism, Qualification, Review
...
class PsychologistSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Psychologist
        fields = [
            'id', 'user', 'user_name', 'user_email', 'title', 'qualifications',
            'specialisms', 'languages', 'service_type', 'service_type_display',
            'modality', 'modality_display', 'location', 'location_lat', 'location_lon',
            'address_line_1', 'address_line_2', 'city', 'postcode', 'country',
            'registration_number', 'registration_body', 'years_experience',
            'availability_status', 'availability_status_display', 'max_patients',
            'current_patients', 'hourly_rate', 'session_duration',
            'preferred_age_groups', 'preferred_conditions', 'max_distance_km',
            'is_verified', 'is_active', 'is_accepting_referrals', 'capacity_available',
            'is_available', 'average_rating', 'review_count',
            'created_at', 'updated_at', 'last_updated_embedding'
        ]
...
class AvailabilitySerializer(serializers.ModelSerializer):
    ...
    """
    psychologist_name = serializers.CharField(source='psychologist.user.get_full_name', read_only=True)
    ...
    class Meta:
        model = Availability
        fields = [
            'id', 'psychologist', 'psychologist_name', 'day_of_week', 'day_of_week_display',
            'start_time', 'end_time', 'modality', 'modality_display', 'is_active',
...
class SpecialismSerializer(serializers.ModelSerializer):
    ...
    def validate_name(self, value):
        ...
        if Specialism.objects.filter(name=value).exists():
            raise serializers.ValidationError("A specialism with this name already exists.")
        return value
...
class QualificationSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Qualification
        fields = [
            'id', 'name', 'description', 'abbreviation', 'is_active',
            'created_at', 'updated_at'
        ]
        read_only_fields = ['id', 'created_at', 'updated_at']
    ...
    def validate_name(self, value):
        ...
        if Qualification.objects.filter(name=value).exists():
            raise serializers.ValidationError("A qualification with this name already exists.")
        return value
...
class ReviewSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Review
        fields = [
            'id', 'psychologist', 'psychologist_name', 'patient', 'patient_name',
            'rating', 'rating_display', 'title', 'content', 'is_verified',
...
class PsychologistProfileSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Psychologist
        fields = [
            'title', 'qualifications', 'specialisms', 'languages',
            'service_type', 'modality', 'address_line_1', 'address_line_2',
            'city', 'postcode', 'country', 'registration_number',
            'registration_body', 'years_experience', 'hourly_rate',
            'session_duration', 'preferred_age_groups', 'preferred_conditions',
            'max_distance_km'
        ]
    ...
    def validate_session_duration(self, value):
        ...
        if value < 15 or value > 480:  # 15 minutes to 8 hours
            raise serializers.ValidationError("Session duration must be between 15 and 480 minutes.")
        return value
...
class PsychologistSearchSerializer(serializers.Serializer):
    ...
    """
    service_type = serializers.ChoiceField(choices=Psychologist.ServiceType.choices, required=False)
    modality = serializers.ChoiceField(choices=Psychologist.Modality.choices, required=False)
