...
from .serializers import PsychologistSerializer, AvailabilitySerializer, SpecialismSerializer, QualificationSerializer, ReviewSerializer
...
class PsychologistListView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        queryset = Psychologist.objects.filter(is_active=True, is_accepting_referrals=True)
...
class PsychologistDetailView(LoginRequiredMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['availabilities'] = psychologist.availabilities.filter(is_active=True).order_by('day_of_week', 'start_time')
        context['reviews'] = psychologist.reviews.filter(is_public=True).order_by('-created_at')[:10]
        return context
...
class PsychologistDashboardView(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if user.is_psychologist and hasattr(user, 'psychologist_profile'):
            ...
            context['availabilities'] = psychologist.availabilities.filter(is_active=True).order_by('day_of_week', 'start_time')
            context['recent_reviews'] = psychologist.reviews.filter(is_public=True).order_by('-created_at')[:5]
            context['upcoming_appointments'] = psychologist.appointments_as_psychologist.filter(
...
class AvailabilityListAPIView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        return Availability.objects.filter(psychologist_id=psychologist_id, is_active=True).order_by('day_of_week', 'start_time')
...
class ReviewListAPIView(generics.ListCreateAPIView):
    ...
    def get_queryset(self):
        psychologist_id = self.kwargs.get('psychologist_id')
        return Review.objects.filter(psychologist_id=psychologist_id, is_public=True).order_by('-created_at')
