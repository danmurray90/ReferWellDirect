...
class OrganisationSerializer(serializers.ModelSerializer):
    ...
    organisation_type_display = serializers.CharField(
        source="get_organisation_type_display", read_only=True
    )
        ...
        if step_type == OnboardingStep.StepType.PROFILE_SETUP:
            ...
            for field in required_fields:
                if field not in data:
                    raise serializers.ValidationError(
                        f"{field} is required for profile setup."
                    )
            ...
            for field in required_fields:
                if field not in data:
                    raise serializers.ValidationError(
                        f"{field} is required for organisation setup."
                    )
        ...
        if action == "complete" and not step_data:
            ...
            try:
                ...
                if step.step_type in [
                    ...
                    raise serializers.ValidationError(
                        "This step requires data to complete."
                    )
            except OnboardingStep.DoesNotExist:
                pass
