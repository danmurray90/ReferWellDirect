...
class Organisation(models.Model):
    ...
    organisation_type = models.CharField(
        max_length=30, choices=OrganisationType.choices
    )
    ...
    ods_code = models.CharField(
        max_length=10, blank=True, help_text="NHS Organisation Data Service code"
    )
    ccg_code = models.CharField(
        max_length=10, blank=True, help_text="Clinical Commissioning Group code"
    )
...
class UserOrganisation(models.Model):
    ...
    user = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="user_organisations"
    )
    organisation = models.ForeignKey(
        Organisation, on_delete=models.CASCADE, related_name="user_organisations"
    )
...
class UserOnboardingProgress(models.Model):
    ...
    step = models.ForeignKey(
        OnboardingStep, on_delete=models.CASCADE, related_name="user_progress"
    )
...
class OnboardingSession(models.Model):
    ...
    user = models.OneToOneField(
        User, on_delete=models.CASCADE, related_name="onboarding_session"
    )
    status = models.CharField(
        max_length=20, choices=Status.choices, default=Status.NOT_STARTED
    )
    current_step = models.ForeignKey(
        OnboardingStep, on_delete=models.SET_NULL, null=True, blank=True
    )
    ...
    session_data = models.JSONField(
        default=dict, blank=True, help_text="Session-wide data"
    )
    ...
    def progress_percentage(self):
        ...
        total_steps = OnboardingStep.objects.filter(
            user_type=self.user.user_type, is_active=True
        ).count()
    ...
    def get_next_step(self):
        ...
        next_step = (
            OnboardingStep.objects.filter(user_type=self.user.user_type, is_active=True)
