...
class UserAdmin(BaseUserAdmin):
    ...
    fieldsets = (
        ...
        ('Personal info', {'fields': ('first_name', 'last_name', 'date_of_birth', 'phone', 'profile_picture')}),
        ('Account info', {'fields': ('user_type', 'is_active', 'is_verified', 'last_login')}),
...
class OrganisationAdmin(admin.ModelAdmin):
    ...
    """
    list_display = ('name', 'organisation_type', 'city', 'postcode', 'is_active', 'is_verified', 'created_at')
    list_filter = ('organisation_type', 'is_active', 'is_verified', 'created_at')
    search_fields = ('name', 'email', 'postcode', 'ods_code', 'ccg_code')
    ordering = ('-created_at',)
    
    fieldsets = (
        ('Basic info', {'fields': ('name', 'organisation_type', 'is_active', 'is_verified')}),
        ('Contact', {'fields': ('email', 'phone', 'website')}),
        ('Address', {'fields': ('address_line_1', 'address_line_2', 'city', 'postcode', 'country', 'location')}),
...
class UserOrganisationAdmin(admin.ModelAdmin):
    ...
    search_fields = ('user__email', 'user__first_name', 'user__last_name', 'organisation__name')
    ordering = ('-created_at',)
    ...
    def get_queryset(self, request):
        return super().get_queryset(request).select_related('user', 'organisation', 'created_by')
