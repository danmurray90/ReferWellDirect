...
class UserModelTest(TestCase):
    ...
    def test_user_str_representation(self):
        ...
        user = User.objects.create_user(**self.user_data)
        expected = (
            f"{user.first_name} {user.last_name} ({user.get_user_type_display()})"
        )
...
class OrganisationModelTest(TestCase):
    ...
    def test_create_organisation(self):
        ...
        self.assertEqual(
            org.organisation_type, Organisation.OrganisationType.GP_PRACTICE
        )
...
class UserViewsTest(TestCase):
    ...
    def test_profile_view_unauthenticated(self):
        ...
        self.assertRedirects(
            response, f"{reverse('accounts:signin')}?next={reverse('accounts:profile')}"
        )
...
class UserAPITest(APITestCase):
    ...
    def test_user_detail_api(self):
        ...
        response = self.client.get(
            reverse("accounts_api:user-detail", kwargs={"pk": self.user.id})
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_user_update_api(self):
        ...
        response = self.client.patch(
            reverse("accounts_api:user-detail", kwargs={"pk": self.user.id}), data
        )
        self.assertEqual(response.status_code, 200)
